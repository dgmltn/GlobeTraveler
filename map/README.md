...
dark mode, bundled pre-stripped to US-state + country labels only in
`composeResources/files/basemap-{light,dark}.json`, generated by
`scripts/strip-basemap-labels.py` — do not edit by hand), `MapSwitcher` (top-center
title in the active tracked map's accent color: tap for the map menu + "New map…"
dialog) under a full-width top gradient scrim (map fades out via `fadeInOut`),
